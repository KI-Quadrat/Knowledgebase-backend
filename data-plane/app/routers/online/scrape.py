...
@router.post(
    ...
    description=(
        ...
        "`crawl4ai` (Crawl4AI `POST /md`), or `firecrawl` (Firecrawl `POST /v2/scrape` — requires "
        "`FIRECRAWL_API_KEY`; point `FIRECRAWL_API_URL` at a self-hosted instance for EU data residency). "
        "The other backends and raw httpx remain as automatic fallbacks if the primary fails. The default "
        "can be overridden globally via the `DEFAULT_SCRAPER` env var. |\n"
...
@router.post(
    ...
    description=(
        ...
        "Single API call regardless of site size. Requires `FIRECRAWL_API_KEY`. | Large sites where "
